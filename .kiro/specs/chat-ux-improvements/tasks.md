...
- [x] 1. Requirement 1: Thought Auto-Deletion（思考自動削除）
  - [x] 1.1 Backend: Config model に `auto_delete_threshold_minutes` フィールド追加
  ...
  - [x] 1.2 Backend: thought_repo に削除系関数追加
  ...
  - [x] 1.3 Backend: ThoughtEngine に `cleanup_old_thoughts` メソッド追加
  ...
  - [x] 1.4 Backend: Tauri Command `delete_thought` 追加
  ...
  - [x] 1.7 Frontend: ThoughtView に削除ボタン追加
    - `src/components/thought/ThoughtView.tsx` の各思考エントリに削除ボタン表示
    - `invoke('delete_thought', { id })` 呼び出し → 成功時にリストから除去
    - _Requirements: 1.4_

  - [x] 1.8 Frontend: ConfigStore に `auto_delete_threshold_minutes` 反映
    - `src/stores/config.store.ts` で ThoughtConfig の新フィールドを管理
    - 設定画面（SettingsView）に閾値設定UIを追加（分単位入力 or プリセット選択）
    - _Requirements: 1.1_

- [x] 2. Requirement 2: Thoughts Reflected in Chat（思考のチャット反映）
  - [x] 2.1 Backend: ChatEngine.build_context に思考コンテキスト追加
...
- [x] 3. Checkpoint - バックエンド思考関連確認
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Requirement 4: Chat Regeneration（チャット再生成）
  - [x] 4.1 Backend: ChatEngine に `regenerate` メソッド追加
  ...
  - [x] 4.2 Backend: Tauri Command `regenerate_message` 追加
  ...
  - [x] 4.4 Frontend: ChatStore に `regenerateMessage` アクション追加
  ...
  - [x] 4.5 Frontend: MessageBubble に再生成ボタン追加
    - `src/components/chat/MessageBubble.tsx` のassistantメッセージにregenerateボタン表示（ホバー時）
    - クリック時に `regenerateMessage` 呼び出し
    - _Requirements: 4.3_

- [x] 5. Requirement 5: Stop Generation Button（生成停止ボタン）
  - [x] 5.1 Backend: StreamAbortManager 実装
  ...
  - [x] 5.2 Backend: send_message のストリーミング処理に AbortHandle 統合
  ...
  - [x] 5.3 Backend: Tauri Command `stop_generation` 追加
  ...
  - [x] 5.5 Frontend: ChatStore に `isAbortable` 状態と `stopGeneration` アクション追加
  ...
  - [x] 5.6 Frontend: ChatView/MessageInput に停止ボタン表示
    - `isAbortable` が true の間、入力エリアに停止ボタン表示
    - クリック時に `stopGeneration()` 呼び出し
    - 停止後: ボタン非表示、入力再有効化
    - _Requirements: 5.1, 5.4_

- [x] 6. Requirement 7: User Message Editing（ユーザーメッセージ編集）
  - [x] 6.1 Backend: ChatEngine に `edit_and_resend` メソッド追加
  ...
  - [x] 6.2 Backend: Tauri Command `edit_and_resend` 追加
  ...
  - [x] 6.4 Frontend: ChatStore に `editAndResend` と `editingMessageId` 追加
  ...
  - [x] 6.5 Frontend: MessageBubble に編集ボタンと EditableMessage コンポーネント追加
...
- [x] 7. Checkpoint - バックエンドチャット操作確認
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Requirement 8: Pause Thought/Spontaneous Speech（思考・自発的発話の一時停止）
  - [x] 8.1 Backend: ThoughtEngine に pause/resume 機能追加
...
- [x] 9. Requirement 3: Chat Focus Retention After Send（送信後フォーカス保持）
  - [x] 9.1 Frontend: MessageInput の送信後フォーカス保持
...
- [x] 10. Requirement 10: Configurable Send Key（送信キー設定）
  - [x] 10.1 Backend: Config model に `send_key` フィールド追加
  ...
  - [x] 10.2 Frontend: MessageInput のキーハンドリング改修
  ...
  - [x] 10.4 Frontend: SettingsView に送信キー設定ドロップダウン追加
    - `src/components/settings/SettingsView.tsx` の「一般」タブにドロップダウン追加
    - 選択肢: Enter / Ctrl+Enter / Shift+Enter
    - 変更時に ConfigStore 経由で保存
    - _Requirements: 10.4_

- [x] 11. Requirement 6: Smooth Scrolling During Generation（スムーズスクロール）
  - [x] 11.1 Frontend: ChatView のオートスクロール改善
  ...
  - [x] 11.2 Frontend: スムーズスクロールアニメーション適用
...
- [x] 12. Requirement 9: Markdown Rendering in Chat（Markdownレンダリング）
  - [x] 12.1 依存パッケージインストール
    - `react-markdown`, `remark-gfm`, `rehype-highlight`, `rehype-sanitize` をインストール
    - _Requirements: 9.1_

  - [x] 12.2 Frontend: MarkdownRenderer コンポーネント作成
  ...
  - [x] 12.4 Frontend: MessageBubble に MarkdownRenderer 統合
...
- [x] 13. Checkpoint - フロントエンド主要機能確認
  - Ensure all tests pass, ask the user if questions arise.

- [x] 14. Requirement 11: Delete Button Visibility After Slide Animation（削除ボタン表示修正）
  - [x] 14.1 Frontend: メッセージ削除アニメーション後のボタンアクセシビリティ修正
