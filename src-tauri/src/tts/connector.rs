...
pub trait TTSConnector: Send + Sync {
    ...
    async fn synthesize(&self, text: &str, config: &TTSConfig, voicepeak_path: Option<&str>) -> Result<Vec<u8>, AppError>;

    /// TTS接続テスト
    /// `voicepeak_path`: 全体設定のVoicePeak CLIパス
    async fn test_connection(&self, config: &TTSConfig, voicepeak_path: Option<&str>) -> Result<(), AppError>;
}
...
impl DefaultTTSConnector {
    ...
    async fn synthesize_irodori(&self, text: &str, config: &TTSConfig) -> Result<Vec<u8>, AppError> {
        let handler = IrodoriTTSHandler::new(&self.http_client);
        handler.synthesize(text, config).await
    }

    /// VoicePeak CLIで音声合成
    async fn synthesize_voicepeak(&self, text: &str, config: &TTSConfig, voicepeak_path: Option<&str>) -> Result<Vec<u8>, AppError> {
    ...
    /// VoicePeak接続テスト
    async fn test_voicepeak(&self, config: &TTSConfig, voicepeak_path: Option<&str>) -> Result<(), AppError> {
        let handler = VoicePeakHandler::new();
...
impl TTSConnector for DefaultTTSConnector {
    async fn synthesize(&self, text: &str, config: &TTSConfig, voicepeak_path: Option<&str>) -> Result<Vec<u8>, AppError> {
        match config.provider {
            ...
            TTSProvider::Voicepeak => self.synthesize_voicepeak(text, config, voicepeak_path).await,
        }
    }
