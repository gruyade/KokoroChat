...
impl DefaultTTSConnector {
    ...
    async fn synthesize_voicepeak(&self, text: &str, config: &TTSConfig) -> Result<Vec<u8>, AppError> {
        let handler = VoicePeakHandler::new(&self.http_client);
        handler.synthesize(text, config).await
    }
...
impl TTSConnector for DefaultTTSConnector {
    async fn synthesize(&self, text: &str, config: &TTSConfig) -> Result<Vec<u8>, AppError> {
        match config.provider {
            ...
            TTSProvider::Voicepeak => self.synthesize_voicepeak(text, config).await,
        }
    }
    ...
    async fn test_connection(&self, config: &TTSConfig) -> Result<(), AppError> {
        match config.provider {
            ...
            TTSProvider::Voicepeak => self.test_voicepeak(config).await,
        }
    }
}
